...
- `server.py` — Flask app serving the discussieboom REST API (`/api/arguments` GET/POST, `/api/arguments/<id>/status` PATCH, `/api/sources`, `/api/citations` POST, plus POST/DELETE for entities/relations/roles/mechanisms/instantiations), `/api/health` (M0.5, reuses `validation.py`), `/api/scores`, and the visualisation at `/`. Opens a fresh SQLite connection per request with `PRAGMA foreign_keys = ON`. Argument creation is logged to `edit_log`.
- **Identity & gates (M0.6).** Reading is open; *every write endpoint requires a user* (table `users`, managed via `scripts/create_user.py`; login portal at `/login`, account page + token rotation at `/account`). Auth is session (`/api/login`, humans with password) or `Authorization: Bearer <token>` (agents and Claude Code; token files live in `data/tokens/`, gitignored, only the sha256 hash is stored). Attribution always follows the logged-in user — payload `contributed_by`/`changed_by` fields are ignored. Role ladder `bijdrager < reviewer < maintainer`: status PATCH needs reviewer+; theory-layer writes and all DELETEs need maintainer. Verifying your own argument is allowed but sets the `self_merged` flag (re-auditable). **Citatiepoort (M0.3):** a supporting/contradicting **root** argument posted without citations starts as `bronvermelding_nodig`; the first citation (or inline `citations` in the POST) promotes it to `ongecontroleerd`. Only `contextual` may start without a source. **Reply rules (M1.1/M1.8):** a reply carries no target and no property (400 otherwise); contradicting replies (ondergravingen) may be sourceless and can carry an `objection_type` (drogreden taxonomy; see `missies/monitor_brief.md`). Agent missions (monitor/scout) run as Claude Code sessions under their own agent accounts with briefs + audit logs in `missies/` (M1.8/M1.9).
