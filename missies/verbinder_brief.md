# Missie-brief: verbinder-agent (structuur-ontdekking — nieuwe knopen & verbanden)

**Account:** `verbinder-agent` (Bearer-token in `data/tokens/verbinder-agent.token`).
...
beargumenteren. Waar de scout bronnen oogst en de monitor/red team bestaande claims
...
## Onderwerpkeuze & anti-overfit-protocol (verplicht — zelfde geest als de scout)
...
2. **Nieuwe entiteit (knoop), als die ontbreekt.** `POST /api/entities` met:
   ...
   - optioneel `primary_role_id` (fallback-rol) en `active_from`/`active_until` als de
     speler aantoonbaar tijdgebonden is.
